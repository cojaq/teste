...
def main():
    ...
    list_actions = []
    actions = jobExec.selectFilterActions("PERSON")
    ...
    df_stg_versions = spark.table(f"{jobExec.database_work}.stg_versions").filter(
        ...
        & (f.col("item_type") == "Person")
        & (f.upper(f.col("action")).isin(list_actions))
    )

    df_dim_person_actions = df_stg_versions.select(
        f.col("item_id").alias("person_id"),
    ...
    df_dim_person_actions = dataframe_utils.createPartitionColumns(
        df_dim_person_actions, jobExec.reference_date
    )

    df_dim_person_actions = jobExec.select_dataframe_columns(
        spark, df_dim_person_actions, table_columns
    )
    df_dim_person_actions = df_dim_person_actions.repartition(
        num_partitions, "person_id"
    )

    df_dim_person_actions.write.insertInto(
